...
try: 
	aux1 = psutil.net_io_counters(pernic=True)['eth0'][0]
	aux2 = psutil.net_io_counters(pernic=True)['eth0'][1]

	while True:

		# outfile_CPU.write(str(psutil.cpu_percent(interval=1))+'\n')
		
		valor1 = psutil.net_io_counters(pernic=True)['eth0'][0] - aux1 
		aux1 = psutil.net_io_counters(pernic=True)['eth0'][0]
		
		valor2 = psutil.net_io_counters(pernic=True)['eth0'][1] - aux2 
		aux2 = psutil.net_io_counters(pernic=True)['eth0'][1]
